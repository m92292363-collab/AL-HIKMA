...
export const handler = async (event) => {
  ...
  let student_id, full_name, email, year, password;
  try {
    ({ student_id, full_name, email, year, password } = JSON.parse(event.body || '{}'));
  } catch {
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ success: false, message: 'Invalid request body' }) };
  }

  if (!student_id || !full_name || !email || !year || !password)
  ...
  try {
    ...
    // Faculty and department come from the JWT token
    // Department staff cannot override these values
    await sql`
